...
const PostHeader = ({ id, author, createdAt, isAReply }) => {
  ...
  return (
    <header className={`${styles["post__header"]}`}>
      ...
      <Link className={styles["header__link"]} onClick={onClickLink} href={`/profile/${author.id}`}>
        <span className={styles["header__username"]}>@{author.username}</span>
      </Link>

      <span className={styles["header__time"]}>{"dateFormat"}</span>

      {/* {author.id === userId && <HeaderFeatures id={id} authorId={author.id} isAReply={isAReply} />} */}
    </header>
  );
};
